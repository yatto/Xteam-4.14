...
#include <mach/rk29_board.h>  //riga inserita perchè ho commentato la riga successiva
#include <mach/gpio.h>
#include "rt5631.h"
...
#endif
#define RT5631_ALC_DAC_FUNC_ENA RT5631_ENABLE_ALC_DAC//0	//ALC functio for DAC
...
static int last_is_spk = 0xff;	// need modify.
#endif
...
/*
#ifndef RT5631_DEF_VOL
#define RT5631_DEF_VOL					0xc2
#endif
#ifndef RT5631_DEF_VOL_SPK
#define RT5631_DEF_VOL_SPK				0xc6
#endif
#ifndef RT5631_ADC_GAIN
#define RT5631_ADC_GAIN				0x0004
#endif
#ifndef RT5631_DEF_VOL_ADC
#define RT5631_DEF_VOL_ADC		0x4400
#endif
*/
#ifndef RT5631_DEF_VOL_LEFT
#define RT5631_DEF_VOL_LEFT	RT5631_DEF_VOL
#endif
#ifndef RT5631_DEF_VOL_RIGHT
#define RT5631_DEF_VOL_RIGHT	RT5631_DEF_VOL
#endif

#ifndef RT5631_DAC_ATTENU
...
static struct rt5631_init_reg init_list[] = {
	...
	{RT5631_SPK_OUT_VOL			, (RT5631_DEF_VOL_SPK<<8) | RT5631_DEF_VOL_SPK},		//speaker channel volume select SPKMIXER,0DB by default
	{RT5631_HP_OUT_VOL			, (RT5631_DEF_VOL_LEFT<<8) | RT5631_DEF_VOL_RIGHT},		//Headphone channel volume select OUTMIXER,0DB by default
	...
	{RT5631_ADC_CTRL_1			, RT5631_ADC_GAIN},//0x0000},//0x0006},//STEREO ADC CONTROL 1
	{RT5631_MIC_CTRL_2			, RT5631_DEF_VOL_ADC},//0x8800},//0x5500}, 		//Mic1/Mic2 boost 40DB by default
	...
	{RT5631_GEN_PUR_CTRL_REG	, 0x4e00},//0x4e00},//Speaker AMP ratio gain is 1.27x
...
#endif
	{RT5631_STEREO_DAC_VOL_2	, RT5631_DAC_ATTENU},//0x0001},		//just for a7htc, r channel -0.375db
};
...
#ifndef RT5631_DEF_HP_EQ
#define RT5631_DEF_HP_EQ	NORMAL
#endif

#ifndef RT5631_DEF_SPK_EQ
#define RT5631_DEF_SPK_EQ   SPK_FR
#endif
...
/*			0x0		0x1		0x2	  0x3	0x4		0x5		0x6		0x7	0x8		0x9		0xa	0xb		0xc		0xd		0xe		0xf		0x6e*/
	...
	{SPK_FR6,"SPK_FR6",{0x1EBD,0xF5AE,0xC4DF,0x1bbc,0x0304,0xC66C,0x19CA,0xF65F,0xDAEE,0x10FB,0xFB54,0x1C8B,0x0000,0xc3DE,0x1C5B,0x1E1F},0x409B},//from jerom, for m907
...
static int curr_eq_mode=RT5631_DEF_SPK_EQ;
static void rt5631_update_eqmode(struct snd_soc_codec *codec, int mode)
{
	int i;
	curr_eq_mode=mode;
	printk("%s %s(), mode=%d(%s)\n", __FILE__, __FUNCTION__, mode, get_eq_name_by_mode(mode));
	if (NORMAL == mode) {
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("%s %s(), set %d\n", __FILE__, __FUNCTION__, ucontrol->value.enumerated.item[0]);
...
static void spk_work_handler(struct work_struct *work)
{
	...
	is_spk = rt5631_read(codec, 0x4a);
	is_spk = is_spk & (0x01<<2);//0x04; //detect rt5631 reg4a[3], 1'b:SPK, 0'b:HP ;
...
#endif		
	if(is_spk && last_is_spk!=1){
#if 1
		on_off_spk(is_recording);
#endif
		rt5631_write_index_mask(codec,0x11,0x0000,0x0007);	//0db
		rt5631_write_index(codec,0x12,0x0003);			//0db
		rt5631_update_eqmode(codec, RT5631_DEF_SPK_EQ);		// SPK is in use, enable EQ mode of RT5631_DEF_SPK_EQ.
		printk("change to spk, eq=%d(%s)\n", RT5631_DEF_SPK_EQ, get_eq_name_by_mode(RT5631_DEF_SPK_EQ));
	}
	else if(!is_spk && last_is_spk!=0){
	//flove071311	rt5631_update_eqmode(codec, NORMAL);		// HP is in use, enable EQ mode of NORMAL.
		...
		rt5631_update_eqmode(codec,RT5631_DEF_HP_EQ);		
		printk("change to hp, eq=%d(%s)\n", RT5631_DEF_HP_EQ, get_eq_name_by_mode(RT5631_DEF_HP_EQ));
	}
	last_is_spk = is_spk;
}
...
//	tmpvol2 = voltab[1][vollevel];
	tmpvol1=get_vol(0x27, RT5631_DEF_VOL_SPK&0x3f, 16, vollevel);
	tmpvol2=get_vol(0x1f, RT5631_DEF_VOL_LEFT&0x1f, 16, vollevel);
        tmpvol3=get_vol(0x1f, RT5631_DEF_VOL_RIGHT&0x1f, 16, vollevel);

	if(vollevel == 0){
		rt5631_write_mask(codec, RT5631_SPK_OUT_VOL, 0x8080, 0x8080);
		rt5631_write_mask(codec, RT5631_HP_OUT_VOL, 0x8080, 0x8080);
	}
#if defined(RTL5631_CONFIG_ADJUST_VOL_BY_CODEC)	
	else{
	...
	rt5631_write_mask(codec, RT5631_HP_OUT_VOL,  ((tmpvol2<<8)|tmpvol3), 0x3f3f);
}
...
static int rt5631_probe(struct snd_soc_codec *codec)
{
	...
	//rt5631_write_index(codec, 0x48, 0xF428);
...
#if (RT5631_SPK_TIMER == 1)
		last_is_spk = 0xff;	//wired~, update eqmode right here by spk_timer.
#endif
/*
	YATTO : Ho Commentato questa sezione perchè altrimenti ho il warning GPIO not requested nel dmesg
*/
	gpio_pull_updown(GPIO_SPK_CON, 0);//important
	gpio_direction_input(GPIO_SPK_CON);   
...
static ssize_t rt5631_show(struct device *dev, struct device_attribute *attr, char *_buf)
{
	return sprintf(_buf, "%s(): curr_reg 0x%04x=0x%04x\n", __FUNCTION__, cur_reg, 
...
static ssize_t rt5631_store(struct device *dev,
...
static struct device *rt5631_dev = NULL;
static struct class *rt5631_class = NULL;
static DEVICE_ATTR(rt5631, 0666, rt5631_show, rt5631_store);
		    ...
		    const struct i2c_device_id *id)
{
	...
	rt5631_dev = device_create(rt5631_class, NULL, MKDEV(0, 1), NULL, "dev");
	device_create_file(rt5631_dev, &dev_attr_rt5631);
/*
	YATTO : Ho Commentato questa sezione perchè altrimenti ho il warning GPIO not requested nel dmesg
*/
	gpio_pull_updown(GPIO_SPK_CON, 0);//important
...
static int __init rt5631_modinit(void)
{
    ...
    if(i2c0_prober_verify(0x1a, 0x7c, 1, 2, 0x10ec) != 0) {
        printk("%s(): Ping error with 0x1a\n", __FUNCTION__);
        return -ENODEV;
    }
    else
        printk("%s(): Ping OK with 0x1a\n", __FUNCTION__);
    set_sound_card_exist(0x5631);
    set_codec_set_spk(rt5631_codec_set_spk);
